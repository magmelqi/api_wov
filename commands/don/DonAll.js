          ...
          for(let i=0; i < DOr.length; i++)  { var n = n + 1
            ...
            try {
              var Gemme = JSON.parse(readFileSync(`././Information/Gemme/Member-Id/${OrO}`, 'utf-8'))} catch (err) {}
                  ...
                  try {
              if (Or.Or > 1 && Gemme.Gemme > 1) {embed.addFields({name: '- - - - - - - - - - - - - - - - - - - - - - - - - -', value:`\`${Or.Pseudo}\` posséde: \`${Or.Or}\` Or et \`${Gemme.Gemme}\` gemme`});}
              else if (Gemme.Gemme > 0) {embed.addFields({name: '- - - - - - - - - - - - - - - - - - - - - - - - - -', value: `\`${Or.Pseudo}\` posséde: \`${Gemme.Gemme}\` gemme`});}
              else if (Or.Or > 0 ) {embed.addFields({name: '- - - - - - - - - - - - - - - - - - - - - - - - - -', value: `\`${Or.Pseudo}\` posséde: \`${Or.Or}\` Or`});}
                  }catch (err) {}
            var Or = 0; var Gemme = 0
          }; embed.addFields({name: '- - - - - - - - - - - - - - - - - - - - - - - - - -', value: `${n} ayant une ressource en banque`})
